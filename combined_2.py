# -*- coding: utf-8 -*-
"""
Created on Sun Dec 02 20:23:02 2018
...
def table_column(table_lines,columns,textb_line_label,block_count, block_bounds):
    ...
    for i in range(0,len(line_left)):
        ...
        min_d = np.min(np.abs(line_done - line_left[i]))
...
def col_bounds(block_column,block_bounds):
    ...
    for i in range(0,n_col):
        ...
        t = 0
        b = 0
        l = np.min(bounds_mat[:,0]) - l_extra
        r = np.max(bounds_mat[:,1]) + r_extra
        t = np.min(bounds_mat[:,2]) - t_extra
...
def block_to_text(block_column,strings,index, block_bounds, image, image2):
    ...
    for i in range(0, len(strings_d)):
        strr = strings_d[i].encode(encoding="utf-8", errors="ignore")
...
#        cv2.imwrite('blck_img.jpg', img)
        if img.shape[0] and img.shape[1]:
            strr = text_it(img)
            strr = strr.encode(encoding="utf-8", errors="ignore")
        else:
            strr = ' ' 
#        print strr
...
def block_check(block_bounds, comp_textb_label,thresh):
...
def block_form(nlabels,consc_det,consc_dist_hist):
    n_blocks1 = 0
    ...
    for i in range(0,non_textb_l.shape[0]):
        ...
        if comp_textb_label_1[start] == -1:
            ...
            while consc_det[start,0] != -1 and comp_textb_label_1[consc_det[start,0]] == -1:
                if consc_det[start,1] < consc_dist_hist:
...
def line_removal(img_skew_corrected):
    ...
    return np.uint8(img_line_only), np.uint8(img_line_removed) , np.uint8(vertical_line) , np.uint8(horizontal_line)
...
def text_inversion(img):
    ...
    for i in range (0,stats_outer_bound.shape[0]):
            ...
            if area_outer_bound[i] < (a*b*2000)/(720*1080.) or area_outer_bound[i]>a*b/1.1:
    ...
    for k in range (0,area_outer_bound.shape[0]):
            if area_outer_bound[k]!=0:
                ...
                for i in range (top-4, stats_outer_bound[k,3]+top+4):
                    for j in range (left-4, stats_outer_bound[k,2]+left+4): 
                        img[i,j] = 255
                
                for i in range (top+1, stats_outer_bound[k,3]+top-1):
                    for j in range (left+1, stats_outer_bound[k,2]+left-1): 
                        img[i,j] = original_img_reverse[i,j]
...
def main_table(input_im, row_print):
    input_im1, ver_line, hor_line = line_removal(input_im)[1:4]
    input_im1 = text_inversion(input_im1)
#    cv2.imwrite('inverted.jpg',input_im1)
    input_imx = copy.copy(np.uint8(input_im1))
#    cv2.imwrite('inputim1.jpg',np.uint8(input_im1))

    val,input_bw=cv2.threshold(np.uint8(input_imx) ,100, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
...
#    cv2.imwrite('after_2.jpg',input_bw)
    ...
    input_bw = cv2.GaussianBlur(input_bw , (3,3) ,0)
#    cv2.imwrite('after_3.jpg',input_bw)
    ...
    consc_dist_hist = plt.hist(consc_dist)[1][1]
    #consc_dist_hist = 10

    comp_textb_label, n_blocks = block_form(nlabels,consc_det,consc_dist_hist)
    ...
    comp_textb_label,chck_lbls = block_check(block_bounds,comp_textb_label,consc_dist_hist)
...
def header_row(table_lines,line_bounds, line_bounds_hor):
...
def header_cells(table_lines, header_row, n_columns, block_count,textb_line_label, block_bounds):
    index = np.argwhere(table_lines == header_row)[0][0]
    header_rows = table_lines[0 : index + 1]
    ...
    if check_2.shape[0]:
        ...
        if index_2 < index_1 :
            ...
            for j in range(index_2 + 1, check_2.shape[0]):
                ind = check_2[j]
                if ind < index_1:
                    h_cells = headers[j]
...
def main_table_1(input_im, row_print):
    input_im1, ver_line, hor_line = line_removal(input_im)[1:4]
    input_imx = copy.copy(np.uint8(input_im1))
#    cv2.imwrite('inputim1.jpg',np.uint8(input_im1))
...
#    comp_textb_label = blck_noise(block_bounds, comp_textb_label)
    ...
    for i in range(0, num_tables):
        ...
        if table.shape[0] > 2:
            block_column = table_column(table,n_colmns,textb_line_label,block_count, block_bounds)
#            block_table_line = align_table(table,n_colmns, block_count,textb_line_label, block_bounds)
            header_index = header_row(table, line_bounds, line_bounds_hor)
            h_cells = header_cells(table, header_index, n_colmns, block_count, textb_line_label, block_bounds)
...
#images = [cv2.imread(file) for file in glob.glob("C:\Axis AI Challenge @ Akash_Abhishek\INPUT FILES*.jpg") ]
...
for i in range(0, len(folders)):
    ...
    images1 = [cv2.imread(file) for file in glob.glob("C:\Axis AI Challenge @ Akash_Abhishek\PROCESSED FILES 2\{}\*.jpg" .format(i+1)) ]    
    images2 = [cv2.imread(file) for file in glob.glob("C:\Axis AI Challenge @ Akash_Abhishek\PROCESSED FILES 2\{}\Detected Tables\*.jpg" .format(i+1)) ]
    row_print = 0
    path_out_i = path_out + '\output_' + str(i + 1) + '.xlsx'
    workbook = xlsxwriter.Workbook(path_out_i)
    worksheet = workbook.add_worksheet() 
    for j in range(0, len(images1)):
        img_j = images1[j]
        row_print = main_table(img_j, row_print)
